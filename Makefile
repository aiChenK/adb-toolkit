...
build-windows: build-web ## 交叉编译 Windows (amd64)
	...
	@mkdir -p $(BIN_DIR)
	@CGO_ENABLED=0 GOOS=windows GOARCH=amd64 GOTOOLCHAIN=$(WINDOWS_TOOLCHAIN) go build -trimpath $(if $(WINDOWS_LDFLAGS),-ldflags="$(WINDOWS_LDFLAGS)",) -o $(BIN_DIR)/$(BIN_PREFIX)-windows-amd64.exe .
